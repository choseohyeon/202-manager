...
- **style.css 수정 시 반드시 N 증가** (현재 v=44)
...
- **summary 카드 없음**: 대기 수는 탭 배지, 위험/반납은 테이블 행 색상으로 표시 — 별도 집계 카드 불필요
- **출석 현황 상태 표시**: 행 배경색으로만 표시 (노란색=위험, 빨간색=자리 반납 대상). statusBadge 코드는 dead code로 제거됨
- **"박탈" → "자리 반납"**: 과격한 표현 완화. 사용자에게 노출되는 문구는 각주와 비활성화 확인 팝업 2곳
- **비활성화 vs 삭제**: 비활성화(`is_active: 0`)는 기록 보존 + 재활성화 가능. 삭제는 출석 기록까지 완전 제거. 자리 반납 처리 시 비활성화 사용
