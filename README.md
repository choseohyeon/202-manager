...
- **관리자 대시보드** — 신청 승인/삭제, 수동 출석 추가, Excel 내보내기
...
### 2026-08-19
- **버그 수정**: 비활성화/삭제 버튼이 onclick 속성 내 따옴표 충돌로 동작 안 하던 문제 수정
- **버그 수정**: 공휴일이 포함된 주가 현재 주가 지난 뒤 통계 테이블에서 사라지던 문제 수정
- **개선**: 출석 날짜 칩(day chips)이 가로로 표시되던 문제 수정 → 세로 정렬
- **개선**: my.html에서 미해당(na) 상태가 `-`로 표시되던 문제 수정 → `미해당`으로 통일
- **배포 URL 변경**: `202-manager.vercel.app` → `cpse202.vercel.app`
